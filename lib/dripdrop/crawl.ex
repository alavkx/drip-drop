defmodule Dripdrop.Crawl do
  ...
  defp parse_product_info({body, path}) do
    ...
    image_urls =
      body
      |> Floki.find(".product-image img")
      |> Enum.map(fn x -> x |> Floki.attribute("src") |> List.first() end)

    image_ids =
      body
      |> Floki.find(".product-image .sequence-number")
      |> Enum.map(&Floki.text/1)

    captions =
      body
      |> Floki.find(".product-image .caption")
      |> Enum.map(&Floki.text/1)

    %{
      skus: skus,
      images: Enum.zip([image_urls, image_ids, captions]),
      product: %{
  ...
  defp get_or_insert_product(%{skus: skus, images: _images, product: product}) do
